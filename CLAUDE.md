...
**`apps/api`** — Express + `mysql2/promise`, ESM (imports com extensão `.js`), sem ORM: SQL parametrizado direto. Rotas em `src/routes/*` (montadas em `src/app.ts`), regra de negócio em `src/services/*`. Escritas em mais de uma tabela usam `withTransaction` de `src/config/database.ts`. O pool usa `timezone: '-03:00'` (Brasília, não UTC) — API e banco ficam em regiões diferentes, e sem isso `DATETIME`/`DATE` gravariam e leriam com o fuso trocado. Data construída à mão em código de aplicação (`new Date(ano, mes, dia)`, sem passar pelo driver) NÃO herda esse fuso — ela usa o do PROCESSO Node, que normalmente é UTC em produção e pode ser outro em dev. Serializada para o front e exibida com `toLocaleDateString` (fuso do navegador, Brasília) isso desalinha o dia exibido. Regra: data pura calculada em código usa `Date.UTC` na construção e getters `getUTC*` na leitura, ponta a ponta — nunca o construtor local nem `toLocaleDateString` sem `timeZone: 'UTC'` explícito.
